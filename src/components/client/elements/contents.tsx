...
export function ContentsSET({ contentTitle }: { contentTitle: string }) {
  ...
  return (
    <>
      {selectedContents &&
        selectedContents.map((content: any, index: number) => (
          <CardContents durationPls={50 * index} key={index}>
            <CardHeader>
              <CardTitle>
                {_localesContent(content.title, content.title_en)}
              </CardTitle>
              <CardDescription
                className="whitespace-pre-line"
                dangerouslySetInnerHTML={{
                  __html: _localesContent(
                    content.description,
...
export function ContentsGET_ModAll({ mode }: { mode: "all" | "list" }) {
  ...
  const [select_project_type, set_select_project_type] =
  ...
  const getFilteredMods = () => {
    return contentObj_all.filter((mod: any) => {
      const searchMatch =
        select_search === "All" ||
        mod.title.toLowerCase().includes(select_search.toLowerCase());
      const modTypeMatch =
        select_mod_type === "All" ||
        mod.tags?.some((tag: any) => tag.label === select_mod_type);
      const versionMatch =
        select_version === "All" || mod.version === +select_version;
      const typeMatch =
        select_project_type === "All" ||
        mod.project_type === select_project_type;
      ...
      <div className="flex flex-col gap-2">
        {contentObj_list.map((list: any, index: number) => (
          <CardContents durationPls={50 * index} key={index}>
            <CardHeader>
              <CardTitle>
                {_localesContent(list.title, list.title_en)}
              </CardTitle>
              <CardDescription
                className="whitespace-pre-line"
                dangerouslySetInnerHTML={{
            ...
            <CardContent className="flex flex-wrap justify-start py-3 px-7 gap-2">
              {contentObj_all.map((mod: any, index: number) => (
                <ModCard
                  key={index}
                  projectId={
                    +_localesContent(mod.project_id, mod.project_id_en)
                  }
                  className={`${
                    mod.project_type === list.group_type ? `` : `hidden`
                  }`}
                >
                  {mod.project_type === list.group_type ? (
                    <>
                      <ModCardHeader>
                        <Image
                          src={`https://uploads.scratch.mit.edu/get_image/project/${_localesContent(
                            mod.project_id,
                            mod.project_id_en
                          )}_480x360.png`}
                          alt="mod image"
                      ...
                      <ModCardContent>
                        <CardTitle>
                          {_localesContent(mod.title, mod.title_en)}
                        </CardTitle>
                        <CardDescription>
                          {_localesContent(mod.description, mod.description_en)}
                        </CardDescription>
                      </ModCardContent>
                      <ModCardFooter>
    ...
    <CardContents durationPls={0}>
      <CardHeader>
        <section className="flex flex-row flex-wrap gap-3">
          <Input
            type="text"
            placeholder={_locales("Search for MODs")}
            onChange={(e) => set_select_search(e.target.value)}
          />
...
export function ContentsGET_GameAll({ mode }: { mode: "release" | "alpha" }) {
  const contentObj_all =
    mode === "release"
      ? contentObj_gamesAll.release
      : contentObj_gamesAll.alpha;
    ...
    return contentObj_all.filter((content: any) => {
      const searchMatch =
        ...
        content.version_name
          .toLowerCase()
          .includes(select_search.toLowerCase());
      const versionMatch =
        select_version === "All" || content.version === +select_version;
      ...
      <CardContents durationPls={0}>
        <CardHeader>
          <section className="flex flex-row flex-wrap gap-3">
            <Input
              type="text"
              placeholder={_locales("Search for Games")}
              onChange={(e) => set_select_search(e.target.value)}
            />
            <Select onValueChange={set_select_version}>
              <SelectTrigger className="w-[180px]">
            ...
            : filteredContents.map((content: any, index: number) => (
                <ModCard key={index}>
                  ...
                  <ModCardFooter className="flex flex-wrap gap-2">
                    <Button>
                      <Link
                        href={`https://scratch.mit.edu/projects/${_localesContent(
                          content.project_id,
                          content.project_id_en
                        )}/`}
                        target="_blank"
