...
export function ScratchStudioAds({
  className,
  durationPls,
}: {
  className?: null | string;
  durationPls?: null | number;
}) {
        ...
        <DropdownMenu>
          ...
          >
            <Avatar className="items-center text-center h-10 w-10 rounded-full border border-neutral-500">
              <AvatarImage
                src={ScratchAd.avatar["90x90"]}
                alt={ScratchAd.username}
          ...
          <DropdownMenuContent className="w-56">
            ...
            <DropdownMenuGroup>
              <Link
                href={`https://docs.google.com/forms/d/e/1FAIpQLSczKklgvbiyc3hkmjdELUPyvAL_1kZDkK5vYv83WDV06QqXsA/viewform?entry.1486506055=${_localesContent(
                  "日本語",
                  "English"
                )}&entry.765880011=${ScratchAd.id}`}
                target="_block"
                aria-label="ScratchAds"
              >
                <DropdownMenuItem>
              ...
              <DropdownMenuItem
                onClick={() =>
                  isAdsOpen ? handleOpenAds(false) : handleOpenAds(true)
                }
              >
                {isAdsOpen ? (
                ...
                <span className="text-sm">
                  {isAdsOpen ? _locales("Close") : _locales("Open")}
                </span>
              </DropdownMenuItem>
              <Link
                href={`https://scratch.mit.edu/projects/${ScratchAd.id}/`}
                target="_block"
              >
                <DropdownMenuItem>
              ...
              <Link
                href={`https://scratch.mit.edu/studios/34675120/`}
                target="_block"
              >
                <DropdownMenuItem>
  ...
  if (!ScratchAd) {
    return (
      <CardContents
        ...
        durationPls={durationPls ? durationPls : 0}
      >
          ...
          {isAdsOpen ? (
            <>
              <Link
                href={`https://scratch.mit.edu/projects/${ScratchAd.id}/`}
                target="_block"
                className="flex border bg-card text-card-foreground bg-no-repeat bg-cover bg-center max-w-[800px] min-h-[239.183px] md:min-h-[400px] w-full h-full rounded-lg shadow-md select-none"
                style={{ backgroundImage: `url(${ScratchAd.image})` }}
              ></Link>
              <div className="absolute top-1 left-1 flex items-center gap-1">
                <h2 className="rounded-md bg-neutral-200/90 dark:bg-neutral-800/90 font-semibold md:text-2xl px-[5px] py-[3px] break-all">
                  {ScratchAd.title}
                </h2>
              </div>
