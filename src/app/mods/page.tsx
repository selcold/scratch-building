...
export default function Home() {
    ...
    // mods 読み込み
    const [modsListHtml, setModsListHtml] = useState<JSX.Element[]>([]);
    useEffect(() => {
        const fetchModsGetListHtml = async () => {
            try {
                const result = await ModsGetListHtml();
                if (result) {
                    //console.log(result)
                    setModsListHtml(result);
                } else {
                    console.error('MODデータが取得できませんでした');
                }
            } catch (error) {
                console.error('エラー:', error);
            }
        }
        fetchModsGetListHtml();
    }, [])
                        ...
                        <div className='mt-20 mb-20'>
                            <div className="items-center">
                                <div className="fadeUpTrigger bg-zinc-800 max-w-[800px] mt-[20px] mb-[20px] ml-auto mr-auto p-[20px] md:rounded-[10px] shadow-md">
                                    <h2 className=' font-bold text-2xl'>ビル経営ゲーム MOD</h2>
                                    <p className='mb-[10px]'>
