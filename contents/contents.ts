...
export const contents_json: ContentsJSON = {
  page_not_found: [
    {
      ...
      title_en: "News",
      description:
        "日本時間（JST）2024年3月22日をもってアカウントシステムを廃止しました。これに伴い、すべてのアカウント情報が削除されました。今後は従来のアカウントシステムに代わり、Scratchアカウントでのログインが必要となります。ログインユーザー限定の機能を利用するには、Scratchアカウントでログインしてください。",
      description_en:
  ...
  home: [
    ...
    },
    {
      ...
      description:
        "Masaabu公式Discordサーバーに参加すると、サイトに投稿されたコメントの通知が受け取れるほか、お知らせチャンネルでは最新アップデートの詳細情報を確認できます。",
      description_en:
        "By joining the official Masaabu Discord server, you can receive notifications for comments posted on the site, and in the announcements channel, you can check detailed information about the latest updates.",
      actions: [
        {
  ...
  mods: [
    {
      title: "MOD",
      description:
        "ビル経営ゲームのMODはゲームをより面白くできる拡張機能のようなものです！自分で作ることもできるので是非 [前提] と書いてあるプロジェクトを見てみてください！",
      description_en:
        "The MODs for the building management game are like extensions that can make the game more interesting! You can also create them yourself, so be sure to check out the projects labeled [Prerequisite]!",
    },
  ],
  issues: [
    {
      title: "Issues",
      description:
        "GitHub リポジトリの問題セクションで問題を報告すると、より迅速な解決につながりますが、GitHub アカウントを使用できない場合は、Google フォームを使用して問題を報告してください。",
      description_en:
        "While reporting issues on GitHub repository's Issues section leads to faster resolution, if you are unable to use a GitHub account, please report the issues using Google Form.",
      actions: [
        {
          label: "GitHub issues",
