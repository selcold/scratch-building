...
export function CommentsHTML({
  ...
  obj_comments_Array.forEach((comment) => {
    ...
    // コメントフォーム
    function set_comment_reply_form(
      ComRepFo_mode: string,
      ComRepFo_group: string,
      ComRepFo_id: string,
      ComRepFo_user_tag: string,
      ComRepFo_replyUser: string = ""
    ) {
      const HtmlreplyForm = document.getElementById(
        `comment_reply_${ComRepFo_id}`
      );
      if (HtmlreplyForm) {
        if (HtmlreplyForm.children.length > 0) {
          ...
          if (ComRepFo_mode === "comment") {
            ...
            root.render(
              <div
                id={`comment_reply_form_${ComRepFo_id}`}
                className="relative flex flex-row flex-nowrap justify-between items-start w-full p-1 mt-[2rem] mb-[0.5rem] animate-fade-up animate-once animate-duration-500 animate-delay-0 animate-ease-in-out animate-normal animate-fill-forwards"
              >
                ...
                <div className="flex flex-wrap flex-col justify-center items-center ml-1 w-full bg-card text-card-foreground">
                  <div className="w-full">
                    <div className="w-full">
                      <div>
                        <textarea
                          id={`comment_reply_form_textarea_${ComRepFo_id}`}
                          placeholder={_locales("Write a comment")}
                          className="relative ml-[0.6rem] p-[0.75rem] border-[1px] bg-neutral-100 dark:bg-neutral-900 border-zinc-500 rounded-[0.5rem] text-left box-border"
                          style={{ width: "calc(100% - 0.5rem)" }}
                        ></textarea>
                      </div>
                    </div>
                    <div
                      className="flex flex-wrap items-center justify-start flex-row gap-2 w-full ml-[0.6rem] mt-[0.4rem]"
                      style={{ width: "calc(100% - 0.5rem)" }}
                    >
                      <Button
                        id={`comment_reply_form_button_${ComRepFo_id}`}
                        onClick={() =>
                          replyForm_send_ButtonClick(
                            username,
                            userId,
                            ComRepFo_user_tag,
                            ComRepFo_group,
                    ...
                    <div className="w-full">
                      <div>
                        <textarea
                          id={`comment_reply_form_textarea_${ComRepFo_id}`}
                          placeholder={_locales("Write a comment")}
                          className="relative ml-[0.6rem] p-[0.75rem] border-[1px] bbg-neutral-100 dark:bg-neutral-900 border-zinc-500 rounded-[0.5rem] text-left box-border"
                          style={{ width: "calc(100% - 0.5rem)" }}
                        ></textarea>
                      </div>
                    </div>
    ...
    async function replyForm_send_ButtonClick(
      FoSendBtCl_user_name: string,
      FoSendBtCl_user_Id: string,
      FoSendBtCl_user_tag: string,
      FoSendBtCl_reply_group_id: string,
      FoSendBtCl_reply_id: string = "false",
      FoSendBtCl_replyUser: string
    ) {
      const comment_reply_form_button = document.getElementById(
        `comment_reply_form_button_${FoSendBtCl_reply_id}`
      );
      if (comment_reply_form_button) {
        ...
        const text_replyForm = document.getElementById(
          `comment_reply_form_textarea_${FoSendBtCl_reply_id}`
        ) as HTMLInputElement;
        if (text_replyForm) {
          const comment = text_replyForm.value;

          const validationResult = ScratchComment_Check(
            FoSendBtCl_user_name,
            comment
          );
          if (validationResult.status) {
            if (
              await API_gas_backendApi_new_commentSend(
                ...
                FoSendBtCl_reply_group_id,
                FoSendBtCl_reply_id,
                FoSendBtCl_replyUser
              )
            ) {
      ...
      return (
        <>
          {tag === "developer" ? (
            <>
              <svg
                className="m-auto h-5 w-5 text-blue-500 stroke-white"
            ...
            <>
              {tag === "authentic" ? (
                <>
                  <svg
                    className="m-auto h-5 w-5 text-orange-500 stroke-white"
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
    ...
    function CommentOptionBar({
      mode,
      commentORreply_contents,
    }: {
      mode: "comment" | "reply";
      commentORreply_contents: any;
    }) {
      if (mode === "reply") {
        return (
          <>
            {username ? (
              <>
                <div className="flex flex-wrap gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-500 ease-in-out">
                  <button
                    title={_locales("Copy")}
                    className="tooltip text-zinc-500 hover:text-zinc-400 transition duration-500 ease"
                    onClick={() =>
                      handleCopy(
                        commentORreply_contents.id,
                        `${window.location.origin}${window.location.pathname}?#commentId_${commentORreply_contents.id}`
                      )
                    }
                  >
                    <FontAwesomeIcon icon={faLink} />
                    <span
                      id={`handleCopy_${commentORreply_contents.id}`}
                  ...
                  <AlertDialogCustomButton_NotRelease>
                    <button
                      title={_locales("Report")}
                      className="text-zinc-500 hover:text-zinc-400 transition duration-500 ease"
                    >
                      {_locales("Report")}
                    </button>
                  </AlertDialogCustomButton_NotRelease>
                  <button
                    title={_locales("Reply")}
                    className="inline-block text-zinc-500 hover:text-zinc-400 transition duration-500 ease"
                    onClick={() =>
                      set_comment_reply_form(
                        "comment_reply",
                        comment.id,
                        commentORreply_contents.id,
                        "",
                        commentORreply_contents.author.username
                      )
                    }
                  >
                    {_locales("Reply")}
                    <FontAwesomeIcon icon={faReply} className="text-sm ml-1" />
                  </button>
                </div>
              </>
            ) : (
      ...
      return (
        <>
          <div
            ...
            className={
              className
                ...
                : `flex flex-row flex-nowrap justify-between items-start w-full p-1 group animate-fade-up animate-once animate-duration-500 animate-delay-0 animate-ease-in-out animate-normal animate-fill-forwards`
            }
            style={{
              opacity: opacity,
          ...
          >
            ...
            <div
              className="flex flex-wrap flex-col justify-center items-start gap-1 w-full min-w-[50%] mb-3"
              style={{ opacity: opacity }}
            >
              ...
              >
                ...
                <div className="flex flex-wrap flex-row justify-between items-center pt-2">
                  ...
                  <CommentOptionBar
                    mode="reply"
                    commentORreply_contents={reply}
                  />
                </div>
              </div>
              <div
                id={`comment_reply_${reply.id}`}
                className="flex flex-row flex-wrap justify-around items-center ml-[-3.5rem] w-full"
    ...
    commentsHtml.push(
      <li
        key={comment.id}
        className="relative flex flex-col md:flex-row flex-wrap items-center justify-end w-full"
      >
        <div
          ...
          <div className="flex flex-wrap flex-col justify-center items-start gap-1 w-full min-w-[50%] mb-3">
            ...
            >
              ...
              <div className="flex flex-wrap flex-row justify-between items-center pt-2">
                ...
                <CommentOptionBar
                  mode="comment"
                  commentORreply_contents={comment}
                />
              </div>
            </div>
            <div
              id={`comment_reply_${comment.id}`}
              className="flex flex-row flex-wrap justify-around items-center w-full"
            ></div>
          </div>
        </div>
        <section
          id={`comment_section_${comment.id}`}
          className="flex flex-wrap flex-col md:flex-row justify-center items-center w-11/12"
        >
          {/* 返信コメントが2つ以上ある場合 */}
                  ...
                  if (index === 1 && reply.reply_Id) {
                    return (
                      <CommentReply
                        ...
                        reply={reply}
                        opacity={0.5}
                        className="relative after:absolute after:bottom-0 after:w-full after:h-full after:pointer-events-none"
                        style={{
                          background: `linear-gradient(rgba(230, 240, 255, 0), #e6f0ff)`,
                          backgroundImage: `linear-gradient(rgba(230, 240, 255, 0), var(--card))`,
                        }}
                      >
                        {/* "返信を表示"ボタンを配置 */}
                        <button
                          className="absolute bottom-5 mt-2 p-2 bg-blue-500/90 border-blue-500/50 text-white hover:scale-[102%] hover:drop-shadow-lg active:opacity-80 active:scale-95 w-full border rounded-md shadow-md transition-all duration-300 ease-in-out"
                          onClick={() =>
              ...
              {showRepliesMap[comment.id] && (
                <>
                  ...
                  <div className="flex flex-row flex-nowrap justify-between items-start w-full p-1 group animate-fade-up animate-once animate-duration-500 animate-delay-0 animate-ease-in-out animate-normal animate-fill-forwards">
                    <button
                      className="mt-2 p-2 bg-blue-500/50 border-blue-500/20 text-white hover:bg-blue-500/40 hover:drop-shadow-lg active:bg-blue-500/700 active:scale-95 w-full border rounded-md shadow-md transition-all duration-300 ease-in-out"
                      onClick={() =>
                        setShowRepliesMap((prevState) => ({
                          ...prevState,
                          [comment.id]: false,
...
export function CommentsHtmlContents({
  commentsRes,
  comments,
  userData,
}: {
      ...
      return (
        <Group>
          <li className="animated-slideIn-up p-2 animate-fade-up animate-once animate-duration-500 animate-delay-0 animate-ease-in-out animate-normal animate-fill-forwards">
            <h1 className="text-zinc-400 text-[1.2rem] m-auto">
              {_locales("No comments yet")}
            </h1>
          </li>
        </Group>
      );
        ...
        <li className="relative flex flex-col md:flex-row flex-wrap items-center justify-end w-full">
          <div className="flex flex-row flex-nowrap justify-between items-start w-full p-1 group animate-fade-up animate-once animate-duration-500 animate-delay-0 animate-ease-in-out animate-normal animate-fill-forwards">
            ...
            <div className="flex flex-wrap flex-col justify-center items-start gap-1 w-full min-w-[50%] mb-3">
              ...
              <Skeleton
                className="relative h-15 ml-[0.6rem] mb-1 p-[0.75rem] rounded-[0.5rem] text-left box-border"
                style={{}}
              />
